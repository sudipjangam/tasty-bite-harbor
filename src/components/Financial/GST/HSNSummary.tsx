...
export const HSNSummary: React.FC<HSNSummaryProps> = ({ period }) => {
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['HSN Code', 'Description', 'UQC', 'Quantity', 'Taxable Value', 'Rate', 'CGST', 'SGST', 'IGST', 'Total Tax'];
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold">HSN/SAC Summary</h3>
            <p className="text-muted-foreground">
              HSN code wise summary for {data?.periodLabel}
            </p>
          </div>
        </div>
        <Button variant="outline" onClick={handleExportCSV} disabled={hsnSummary.length === 0}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-gradient-to-br from-green-500/10 to-emerald-500/10 dark:from-green-900/30 dark:to-emerald-900/30">
          <CardContent className="pt-6">
            ...
            <p className="text-2xl font-bold"><CurrencyDisplay amount={totals.taxableValue} /></p>
          </CardContent>
        </Card>
        <Card className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 dark:from-purple-900/30 dark:to-pink-900/30">
          <CardContent className="pt-6">
            <p className="text-sm text-muted-foreground">Total Tax</p>
            <p className="text-2xl font-bold"><CurrencyDisplay amount={totals.totalTax} /></p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Required for invoices with value above ₹2.5 Lakhs</CardDescription>
        </CardHeader>
        <CardContent>
          {hsnSummary.length > 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {hsnSummary.map((hsn, idx) => (
                    <tr key={hsn.hsnCode + idx} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800">
                      <td className="py-3 px-4">
                        <Badge variant="outline" className="font-mono">{hsn.hsnCode}</Badge>
                      </td>
                      <td className="py-3 px-4 max-w-[200px] truncate" title={hsn.description}>
                        {hsn.description}
                      </td>
                      <td className="text-center py-3 px-4">NOS</td>
                      <td className="text-right py-3 px-4">{hsn.quantity}</td>
                      <td className="text-right py-3 px-4"><CurrencyDisplay amount={hsn.taxableValue} /></td>
                      <td className="text-center py-3 px-4">
                        <Badge className={
                          hsn.taxRate === 5 ? 'bg-green-500' :
                          hsn.taxRate === 12 ? 'bg-blue-500' :
                          hsn.taxRate === 18 ? 'bg-purple-500' :
                          hsn.taxRate === 28 ? 'bg-red-500' : 'bg-gray-500'
                        }>
                          {hsn.taxRate}%
                        </Badge>
                      </td>
                      <td className="text-right py-3 px-4"><CurrencyDisplay amount={hsn.cgstAmount} /></td>
                      <td className="text-right py-3 px-4"><CurrencyDisplay amount={hsn.sgstAmount} /></td>
                      <td className="text-right py-3 px-4"><CurrencyDisplay amount={hsn.igstAmount} /></td>
                      <td className="text-right py-3 px-4 font-semibold"><CurrencyDisplay amount={hsn.totalTax} /></td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-gray-100 dark:bg-gray-700 font-semibold">
                    ...
                    <td className="text-right py-3 px-4"><CurrencyDisplay amount={totals.taxableValue} /></td>
                    <td className="text-center py-3 px-4">-</td>
                    <td className="text-right py-3 px-4"><CurrencyDisplay amount={totals.cgst} /></td>
                    <td className="text-right py-3 px-4"><CurrencyDisplay amount={totals.sgst} /></td>
                    <td className="text-right py-3 px-4"><CurrencyDisplay amount={totals.igst} /></td>
                    <td className="text-right py-3 px-4"><CurrencyDisplay amount={totals.totalTax} /></td>
                  </tr>
                </tfoot>
              </table>
            </div>
            ...
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm mt-2">HSN codes are captured from invoice line items</p>
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800">
        <CardContent className="pt-6">
          <h4 className="font-semibold text-amber-800 dark:text-amber-200 mb-3">Common HSN/SAC Codes for Hospitality</h4>
