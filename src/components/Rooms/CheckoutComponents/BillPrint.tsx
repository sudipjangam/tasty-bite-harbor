...
const BillPrint = forwardRef<HTMLDivElement, BillPrintProps>(
  (
    ...
    const safeFormatDate = (
      ...
      if (!dateStr || dateStr === "N/A") return fallback;
      try {
    ...
    const subtotal =
      ...
      foodOrders.reduce((sum, order) => sum + order.total, 0);
    ...
    return (
      ...
      >
        ...
        <div className="text-center mb-1">
          ...
          {restaurantPhone && (
            <p className="mb-0.5" style={{ fontSize: "11px" }}>
              Ph: {restaurantPhone}
            </p>
          )}
          {gstNumber && (
        ...
        <div className="mb-1">
          <p style={{ fontSize: "12px" }}>
            <span className="font-semibold">Bill#:</span> {billId}
          </p>
          <p style={{ fontSize: "12px" }}>
            <span className="font-semibold">To:</span> {roomName}
          </p>
          <p style={{ fontSize: "12px" }}>
            <span className="font-semibold">Date:</span>{" "}
            {safeFormatDate(billDate, "dd MMM yyyy", billDate)}
          </p>
          <p style={{ fontSize: "12px" }}>
            <span className="font-semibold">Time:</span>{" "}
            {safeFormatDate(billDate, "hh:mm a", "--:--")}
          </p>
        </div>
        ...
        <div className="mb-1">
          <p style={{ fontSize: "12px" }}>
            <span className="font-semibold">Guest:</span> {customerName}
          </p>
          {customerPhone && (
            <p style={{ fontSize: "12px" }}>
          ...
          <p style={{ fontSize: "12px" }}>
            ...
            {checkInDate && checkInDate !== "N/A"
              ? safeFormatDate(checkInDate, "dd MMM", checkInDate)
              : checkInDate}{" "}
            -{" "}
            {checkOutDate && checkOutDate !== "N/A"
              ? safeFormatDate(checkOutDate, "dd MMM", checkOutDate)
              : checkOutDate}{" "}
            ({daysStayed}d)
          </p>
        </div>
        ...
        {/* Items Table Header */}
        <div
          className="flex justify-between border-t border-b border-gray-400 py-0.5"
          style={{ fontSize: "11px" }}
        >
        ...
        <div className="text-center mt-2">
          <p className="font-bold mb-1" style={{ fontSize: "11px" }}>
            Thank You!
          </p>
