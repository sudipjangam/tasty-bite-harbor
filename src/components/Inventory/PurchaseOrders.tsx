...
import { Plus, Package, Eye, Check, X, Truck, Edit, Trash2, Search } from "lucide-react";
...
const PurchaseOrders = () => {
  ...
  const totalOrderAmount = orderItems.reduce((sum, item) => sum + (item.quantity * item.unit_cost), 0);
    ...
    <div className="space-y-6">
        ...
        <>
          <div className="grid grid-cols-1 gap-4">
            {paginatedOrders.map((order) => (
              <Card key={order.id} className="p-4 hover:shadow-md transition-shadow bg-white/90 dark:bg-gray-800/90 rounded-xl">
                <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-600 dark:text-gray-400 space-y-1">
                      <p><strong>Supplier:</strong> {order.supplier?.name || "Unknown"}</p>
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto bg-white/95 dark:bg-gray-800/95 backdrop-blur-xl rounded-2xl">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">Expected Delivery Date</Label>
                <Input
                  type="date"
            ...
            <div>
              ...
              {orderItems.length === 0 ? (
                <Card className="p-6 text-center border-dashed bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">No items added. Click "Add Item" to start.</p>
                </Card>
              ) : (
                <div className="space-y-3">
                  {orderItems.map((item, index) => (
                    <Card key={index} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t">
              ...
              <Button 
                ...
                disabled={!selectedSupplierId || orderItems.length === 0 || createOrderMutation.isPending}
                className="bg-gradient-to-r from-purple-500 to-violet-600 hover:from-purple-600 hover:to-violet-700 text-white rounded-xl"
              >
            ...
            <div className="space-y-4">
              <div>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                  Order: <strong className="text-gray-900 dark:text-white">{selectedOrder.order_number}</strong>
                </p>
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  Current Status: <Badge className={statusColors[selectedOrder.status]}>
              ...
              <div>
                <Label htmlFor="new-status" className="text-gray-700 dark:text-gray-300">New Status</Label>
          ...
          {selectedOrder && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-gray-50 dark:bg-gray-700/50 rounded-xl">
                <div>
                  <h4 className="font-semibold text-gray-900 dark:text-white mb-2">Order Information</h4>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {selectedOrder.purchase_order_items?.map((item) => (
                    <div key={item.id} className="border rounded-xl p-4 bg-white dark:bg-gray-700/50">
                      <div className="flex justify-between items-start">
                        <div className="flex-1">
                          <h5 className="font-medium text-gray-900 dark:text-white">{item.inventory_item?.name || "Unknown Item"}</h5>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-2 text-sm text-gray-600 dark:text-gray-400">
                            <div>
                              <span className="font-medium">Ordered:</span>
                              <p>{item.quantity} {item.inventory_item?.unit || "units"}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className={item.received_quantity > 0 ? "text-green-600" : "text-gray-400"}>
                                {item.received_quantity || 0} {item.inventory_item?.unit || "units"}
                              </p>
                            </div>
                          </div>
                        </div>
                        <div className="ml-4">
                          {item.received_quantity >= item.quantity ? (
                            <Badge className="bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-300">Fully Received</Badge>
                          ) : item.received_quantity > 0 ? (
                            <Badge className="bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-300">Partially Received</Badge>
                          ) : (
                            <Badge className="bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">Pending</Badge>
                          )}
                        </div>
                      </div>
                    </div>
      ...
      <Dialog open={isReceivingDialogOpen} onOpenChange={setIsReceivingDialogOpen}>
        <DialogContent className="max-w-2xl bg-white/95 dark:bg-gray-800/95 rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-gray-900 dark:text-white">Receive Items - {selectedOrder?.order_number}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleReceiveItems} className="space-y-4">
            <div className="max-h-96 overflow-y-auto space-y-3">
              {selectedOrder?.purchase_order_items?.map((item) => (
                <div key={item.id} className="border rounded-xl p-3 bg-gray-50 dark:bg-gray-700/50">
                  <div className="flex justify-between items-center mb-2">
                    <h4 className="font-medium text-gray-900 dark:text-white">{item.inventory_item?.name || "Unknown Item"}</h4>
                    <span className="text-sm text-gray-500 dark:text-gray-400">
                      Ordered: {item.quantity} {item.inventory_item?.unit || "units"}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <Label>Unit</Label>
                      <Input value={item.inventory_item?.unit || "units"} disabled className="bg-gray-100 dark:bg-gray-700 rounded-lg" />
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="flex justify-end gap-2">
              <Button type="button" variant="outline" onClick={() => setIsReceivingDialogOpen(false)} className="rounded-xl">
                Cancel
              </Button>
