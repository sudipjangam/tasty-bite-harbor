  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-3 py-2 space-y-2">
        {/* ─── Edit Mode Banner ─── */}
        {isEditMode && (
          <div className="flex items-center justify-between px-2 py-2 mb-1 rounded-xl bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/10 border border-amber-200/50 dark:border-amber-700/30">
            <span className="text-[11px] font-bold text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
              ✏️ Editing Order — add, remove, or adjust items
            </span>
            {onCancelEdit && (
              <button
                onClick={onCancelEdit}
                className="text-[10px] font-bold text-red-400 hover:text-red-500 uppercase tracking-wider px-2 py-0.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
              >
                ✕ Cancel
              </button>
            )}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      item.notes
                        ...
                        : "bg-gray-100 dark:bg-white/10 text-gray-400 dark:text-white/40 hover:text-indigo-500 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10"
                    )}
                    title="Add note"
                  >
            ...
            {editingNoteId === item.id && (
              <div className="mt-2 flex gap-1.5">
                <input
                  ...
                  className="flex-1 px-2.5 py-1.5 text-xs rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400/50"
                  autoFocus
                />
                <button
                  onClick={() => handleSaveNote(item.id)}
                  className="px-2.5 py-1.5 text-[10px] font-bold bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-xl hover:from-indigo-600 hover:to-purple-600 transition-all active:scale-95"
                >
                  Save
                </button>
                <button
                  onClick={() => setEditingNoteId(null)}
                  className="px-2.5 py-1.5 text-[10px] font-bold bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-all active:scale-95"
                >
                  ✕
                </button>
              </div>
