...
const QuickStats = ({ timeRange = "today" }: QuickStatsProps) => {
  ...
  const { data: stats, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Get active orders for the selected time range from unified table
      ...
      const { data: upcomingLeaves } = await supabase
        ...
        .eq("status", "approved")
        .or(
          `start_date.gte.${format(today, "yyyy-MM-dd")},end_date.gte.${format(
            today,
            "yyyy-MM-dd"
          )}`
        );
  ...
  const quickStats = [
    {
      ...
      icon: ClipboardList,
      color:
        "bg-blue-100 dark:bg-blue-900/40 text-brand-deep-blue dark:text-blue-300",
      route: "/orders",
      alert: false,
    },
    {
      ...
      color:
        "bg-green-100 dark:bg-green-900/40 text-brand-success-green dark:text-green-300",
      route: "/tables",
      alert: false,
    },
    {
    ...
    },
    {
      ...
      icon: Users,
      color:
        "bg-purple-100 dark:bg-purple-900/40 text-brand-deep-blue dark:text-purple-300",
      route: "/staff",
      alert: false,
    },
    {
