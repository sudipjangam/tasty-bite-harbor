            ...
            >
              {item.image_url ? (
                <LazyImage
                  ...
                  containerClassName="w-16 h-16 rounded-lg mx-auto mb-2 overflow-hidden"
                />
              ) : (
                <div className="text-4xl mb-2">{item.is_veg ? "🥦" : "🍖"}</div>
              )}
              <div className="font-medium text-card-foreground mb-1 line-clamp-2">
                {item.name}
              </div>
              {item.description && (
              ...
              {item.is_veg !== undefined && (
                <div className="mt-2">
                  <span
                    className={`px-2 py-0.5 text-xs rounded-full ${item.is_veg ? "bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100" : "bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-100"}`}
                  >
