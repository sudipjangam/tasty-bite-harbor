...
export const ConsolidatedRevenueChart = ({
  data,
  showHotelData = true,
}: ConsolidatedRevenueChartProps) => {
  const { formatCurrency, symbol } = useCurrency();

  // Calculate display values based on showHotelData
  const displayTotal = showHotelData
    ? data.grandTotal
    : data.totalRestaurantRevenue;
  ...
  const chartOptions: Options = {
    ...
    xAxis: {
      categories: data.dailyRevenue.map((d) =>
  ...
  return (
    <Card className="p-6 border-0 shadow-lg bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Last 30 days{" "}
              {showHotelData ? "- Restaurant + Hotel combined" : ""}
            </p>
          </div>
        ...
        {/* Revenue breakdown */}
        <div
          className={`grid ${showHotelData ? "grid-cols-2" : "grid-cols-1"} gap-4 mb-6`}
        >
