...
const ExpensesList = () => {
  ...
  const getCategoryColor = (category: string) => {
    ...
    return colors[category] || "bg-gradient-to-r from-gray-500 to-slate-600 text-white";
  };

  const getCategoryLabel = (category: string) => {
    const labels: Record<string, string> = {
      groceries: "Groceries", ingredients: "Ingredients", staff_salary: "Staff Salaries",
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-24 bg-gradient-to-r from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-800 rounded-xl animate-pulse" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {filteredExpenses.length === 0 ? (
          <div className="text-center py-12 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-gray-700">
            ...
            <p className="text-gray-600 dark:text-gray-300 font-medium">No expenses found</p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    <Badge className={`${getCategoryColor(expense.category)} shadow-md`}>
                      {getCategoryLabel(expense.category)}
                    </Badge>
                    {expense.subcategory && (
                      <span className="text-sm text-gray-500 dark:text-gray-400">• {expense.subcategory}</span>
                    )}
                    {expense.is_recurring && (
                      <Badge variant="outline" className="text-xs border-purple-300 dark:border-purple-600 text-purple-600 dark:text-purple-400">
                        Recurring
                      </Badge>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={() => setDeleteExpenseId(expense.id)} className="text-red-600 dark:text-red-400">
      ...
      <ExpenseForm
        ...
        onSuccess={() => { refetch(); queryClient.invalidateQueries({ queryKey: ["expense-data"] }); }}
