...
const OrderItem: React.FC<OrderItemProps> = ({
  ...
  return (
    <div
      className="bg-white/70 backdrop-blur-2xl border border-white/85 rounded-2xl sm:rounded-[20px] mb-3 overflow-hidden transition-all duration-200 hover:-translate-y-0.5 group"
      style={{
    ...
    >
      ...
      {/* ─── CARD HEADER ────────────────────────────────────────────── */}
      <div className="flex items-center justify-between gap-2 px-3 sm:px-4 py-2 bg-slate-50/70 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          {/* Order ID badge */}
          <div
            className="font-mono text-[10px] sm:text-[11px] font-bold px-2 py-0.5 rounded-md border whitespace-nowrap"
            style={{
          ...
          {sourceLabel && (
            <span
              className="text-[10px] sm:text-[11px] font-bold px-2.5 py-0.5 rounded-md tracking-wide whitespace-nowrap"
              style={{
          ...
          {/* Priority badges */}
          {order.priority === "vip" && (
            <div className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 text-white shadow-xs">
              <Star className="w-3 h-3" /> VIP
            </div>
          )}
          {order.priority === "rush" && (
            <div className="flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-gradient-to-r from-red-500 to-pink-500 text-white shadow-xs">
        ...
        {/* Status Pill */}
        <div className={`flex items-center gap-1.5 text-[10px] font-bold px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full border tracking-wide whitespace-nowrap shrink-0 ${statusConfig.bgClass}`}>
          <span
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 sm:p-4">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {order.customer_phone && (
              <a
                ...
                className="text-xs font-semibold text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 px-2 py-0.5 rounded-md flex items-center gap-1 transition-colors"
              >
                📞 {order.customer_phone}
              </a>
            )}
          ...
          <div className="text-[11px] text-slate-400 dark:text-slate-500 font-medium flex items-center gap-1.5 flex-wrap">
            {order.attendant && <span>Attendant: {order.attendant}</span>}
            {order.attendant && <span>·</span>}
        ...
        {/* Right: Total Amount & Payment Badge */}
        <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800">
          <div className="sm:text-right">
            <div className="text-[9px] font-bold tracking-widest uppercase text-slate-400 mb-0.5">Total Amount</div>
            <div className="flex items-center gap-2 sm:justify-end">
              {isNCOrder ? (
                ...
                <CurrencyDisplay
                  amount={order.total}
                  className="text-lg sm:text-[20px] font-extrabold font-mono text-slate-900 dark:text-slate-100 tracking-tight"
                />
              )}
              ...
              {order.payment_method && order.payment_method !== "nc" && (() => {
                ...
                const badgeConfig: Record<string, { bg: string; icon: React.ReactNode; label: string }> = {
                  ...
                  pay_later: { bg: "bg-amber-500 text-white border border-amber-600 shadow-xs", icon: <Clock className="w-3 h-3" />, label: "PAY LATER" },
                ...
                return (
                  <span
                    className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-md tracking-wider flex items-center gap-1 uppercase ${config.bg}`}
      ...
      {/* Items chips */}
      <div className="px-3 sm:px-4 pb-2.5 sm:pb-3 flex gap-1.5 sm:gap-[7px] flex-wrap">
        {parsedItems.map((item, idx) => (
          <div
            key={idx}
            className="flex items-center gap-1 sm:gap-1.5 text-[10.5px] sm:text-[11px] font-medium bg-white/75 dark:bg-slate-800/80 border border-blue-100/60 dark:border-slate-700 rounded-lg px-2 sm:px-2.5 py-1 sm:py-1.5 text-slate-700 dark:text-slate-300 transition-all hover:bg-white dark:hover:bg-slate-800 hover:border-blue-200"
          >
            <span className="font-extrabold text-blue-600 dark:text-blue-400 font-mono text-[9.5px] sm:text-[10px]">{item.qty}×</span>
            <span>{item.name}</span>
            {item.price && <span className="font-mono text-[9.5px] sm:text-[10px] text-slate-400">{item.price}</span>}
          </div>
        ))}
      </div>

      {/* Footer: Time + Actions */}
      <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-2.5 flex-wrap gap-2 border-t border-slate-100 dark:border-slate-800">
        <div className="flex items-center gap-1.5 text-[10.5px] sm:text-[11px] text-slate-400 font-medium">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          ...
          {onPriorityChange && (
            ...
            >
              <SelectTrigger className="h-6 sm:h-7 text-[9.5px] sm:text-[10px] border-slate-200 dark:border-slate-700 w-[88px] sm:w-[100px] rounded-lg">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
