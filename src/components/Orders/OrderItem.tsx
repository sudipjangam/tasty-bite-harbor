...
const OrderItem: React.FC<OrderItemProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-2.5 sm:gap-3 p-3 sm:p-4 pb-2 sm:pb-3">
        {/* Order number tag */}
        <div className="font-mono text-[9.5px] sm:text-[10px] font-semibold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-md border whitespace-nowrap mt-0.5 flex-shrink-0"
          style={{
        ...
        <div className="flex-1 min-w-0">
          <div className="text-sm sm:text-[15px] font-bold text-slate-800 mb-1 tracking-tight truncate">
            {order.customer_name}
          </div>
      ...
      <div className="px-3 sm:px-4 pb-2.5 sm:pb-3 flex gap-1.5 sm:gap-[7px] flex-wrap">
        {parsedItems.map((item, idx) => (
          <div
            key={idx}
            className="flex items-center gap-1 sm:gap-1.5 text-[10.5px] sm:text-[11px] font-medium bg-white/75 border border-blue-100/60 rounded-lg px-2 sm:px-2.5 py-1 sm:py-1.5 text-slate-600 transition-all hover:bg-white hover:border-blue-200"
          >
            <span className="font-extrabold text-blue-600 font-mono text-[9.5px] sm:text-[10px]">{item.qty}×</span>
            <span>{item.name}</span>
      ...
      {/* Footer: Time + Actions */}
      <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-2.5 flex-wrap gap-2 border-t border-slate-100/60">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          ...
          {onPriorityChange && (
            ...
            >
              <SelectTrigger className="h-6 sm:h-7 text-[9.5px] sm:text-[10px] border-slate-200 w-[88px] sm:w-[100px] rounded-lg">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
