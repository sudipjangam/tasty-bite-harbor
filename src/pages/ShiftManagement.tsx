...
import { 
  Clock, Plus, Edit, Trash2, Users, Calendar, Save, X, Check, 
  Sunrise, Sunset, Moon, Loader2, Download, Copy
} from "lucide-react";
...
const ShiftManagement: React.FC = () => {
  ...
  const toggleAssignmentMutation = useMutation({
    mutationFn: async ({ staffId, shiftId, dayOfWeek, isAssigned }: { 
      staffId: string; shiftId: string; dayOfWeek: number; isAssigned: boolean 
    }) => {
      if (isAssigned) {
        // Remove assignment
        const { error } = await supabase
        ...
        // Remove any existing assignment for this staff/day first
        await supabase
          .from('staff_shift_assignments')
          .delete()
          .eq('staff_id', staffId)
  ...
  const getAssignment = (staffId: string, dayOfWeek: number) => {
    return assignments.find(a => a.staff_id === staffId && a.day_of_week === dayOfWeek);
  };
  ...
  const handleBulkAssign = async () => {
    if (!bulkShiftId || selectedStaff.size === 0 || bulkDays.size === 0) {
      toast({ title: 'Please select staff, shift, and days', variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      for (const staffId of selectedStaff) {
        for (const day of bulkDays) {
          // Delete existing assignments for this staff/day
          await supabase
            .from('staff_shift_assignments')
            .delete()
            .eq('staff_id', staffId)
      ...
      toast({ title: `Assigned ${selectedStaff.size} staff to ${bulkDays.size} days` });
      queryClient.invalidateQueries({ queryKey: ['shift-assignments'] });
  ...
  // Export schedule as text/printable
  const exportSchedule = () => {
    const weekStart = startOfWeek(new Date(), { weekStartsOn: 0 });
    let scheduleText = `Staff Schedule - Week of ${format(weekStart, 'MMMM d, yyyy')}\n`;
    ...
    staff.forEach(member => {
      ...
      [0, 1, 2, 3, 4, 5, 6].forEach(day => {
        ...
        if (shift) {
          scheduleText += `  ${dayDate}: ${shift.name} (${formatTime(shift.start_time)} - ${formatTime(shift.end_time)})\n`;
        } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-purple-950">
      <PageHeader 
        ...
        description="Configure shifts and assign staff schedules"
      />
        ...
        <Tabs defaultValue="shifts" className="space-y-6">
          ...
          <TabsContent value="shifts" className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button onClick={() => { resetShiftForm(); setIsShiftDialogOpen(true); }}>
            ...
            {shiftsLoading ? (
              ...
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-gray-500 mb-4">Create your first shift to start scheduling staff.</p>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <Badge style={{ backgroundColor: shift.color }} className="text-white">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>{shift.auto_clock_out_minutes} min after end</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                {shifts.filter(s => s.is_active).map(s => (
                  <Badge key={s.id} style={{ backgroundColor: s.color }} className="text-white text-xs">
                    {s.name}
                  </Badge>
                ))}
              </div>
            ...
            {staff.length === 0 ? (
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-gray-500">Add staff members first to assign shifts.</p>
                </CardContent>
              </Card>
            ) : (
              <Card>
                ...
                {SHIFT_COLORS.map(color => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full transition-all ${
                      shiftForm.color === color ? 'ring-2 ring-offset-2 ring-gray-600 scale-110' : ''
                    }`}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setShiftForm({ ...shiftForm, grace_period_minutes: parseInt(e.target.value) })}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setShiftForm({ ...shiftForm, auto_clock_out_minutes: parseInt(e.target.value) })}
                />
              </div>
            </div>
          </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={bulkAssigning || !bulkShiftId || selectedStaff.size === 0 || bulkDays.size === 0}
            >
              {bulkAssigning ? (
                <Loader2 className="h-4 w-4 animate-spin mr-2" />
              ) : (
