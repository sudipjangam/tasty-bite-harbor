...
const Security = () => {
  ...
  const { data: dbUserPermissions = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.warn("Could not fetch permissions from DB:", error);
  ...
  // Get permissions for current user role (now uses database)
  const getUserPermissions = (): Permission[] => {
    return dbUserPermissions;
  };
  ...
  const getPermissionDescription = (permission: Permission): string => {
    const descriptions: Record<Permission, string> = {
      ...
      "housekeeping.assign": "Assign housekeeping tasks to staff",
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "good":
        return "from-emerald-500/20 to-green-500/20 border-emerald-300 dark:border-emerald-700";
      case "warning":
        return "from-amber-500/20 to-yellow-500/20 border-amber-300 dark:border-amber-700";
      case "error":
        return "from-red-500/20 to-rose-500/20 border-red-300 dark:border-red-700";
      default:
        return "from-gray-500/20 to-slate-500/20 border-gray-300 dark:border-gray-700";
    }
  };
                ...
                return (
                  <Card
                    ...
                    className={`bg-gradient-to-br ${getStatusColor(
                      metric.status
                    )} backdrop-blur-xl border shadow-xl hover:shadow-2xl transition-all duration-300`}
                  >
                    <CardContent className="p-6">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col items-center gap-2">
                          <div
                            className={`p-2.5 bg-gradient-to-br ${metric.gradient} rounded-xl shadow-lg`}
                          >
                            <metric.icon className="h-5 w-5 text-white" />
                          </div>
                          <StatusIcon
                            className={`h-4 w-4 ${getStatusIconColor(
                              metric.status
                            )}`}
                          />
                        </div>
                ...
                {getCurrentRoleData() ? (
                  <div className="space-y-4">
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border border-indigo-200/50 dark:border-indigo-800/50">
                      ...
                      <RoleBadge
                        role={(user.role_name_text || user.role) as UserRole}
                      />
                    </div>
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border border-white/20 dark:border-gray-700/50">
                        <DialogHeader>
                          ...
                          <DialogDescription>
                            Detailed list of all permissions available to your
                            role
                          </DialogDescription>
                        </DialogHeader>
                        <div className="grid grid-cols-1 gap-3 mt-4">
                          {getCurrentRoleData()?.permissionsList.map(
                            (permission, index) => {
                              const IconComponent =
                              ...
                              return (
                                <div
                                  key={index}
                                  className="flex items-start gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-gray-900/50"
                                >
