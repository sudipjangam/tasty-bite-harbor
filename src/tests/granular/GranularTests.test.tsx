...
beforeAll(async () => {
  ...
  if (error) {
    throw new Error(
      `Authentication failed: ${error.message}. Ensure admin@example.com exists with password 'password'.`
    );
  }
...
afterAll(async () => {
  ...
  for (const id of createdIds.orders) {
    await supabase.from("orders_unified").delete().eq("id", id);
  }
  ...
  describe("Role Management (Read)", () => {
    it("should fetch roles list", async () => {
      const { data, error } = await supabase
        .from("roles")
        .select(
          "id, name, description, is_system, has_full_access, is_deletable"
        )
        .eq("restaurant_id", TEST_RESTAURANT_ID);
