...
export const usePerformanceMetrics = () => {
  ...
  return useQuery({
    queryKey: ["performance-metrics", restaurantId],
    queryFn: async (): Promise<PerformanceMetrics> => {
      ...
      const avgRating =
        currentWeekRatings.length > 0
          ? currentWeekRatings.reduce((sum, f) => sum + f.rating, 0) /
      ...
      const prevAvgRating =
        prevWeekRatings.length > 0
          ? prevWeekRatings.reduce((sum, f) => sum + f.rating, 0) /
            prevWeekRatings.length
          : 4.6;
      ...
      const occupancy = totalRooms?.length
        ? ((occupiedRooms?.length || 0) / totalRooms.length) * 100
        : 78;
      ...
      return {
        ...
        customerSatisfactionChange: Number(
          (customerSatisfaction - prevCustomerSatisfaction).toFixed(1)
        ),
        tableTurnoverChange: Number(
          (tableTurnoverRate - prevTableTurnover).toFixed(1)
        ),
