...
export const useRealTimeBusinessData = () => {
  ...
  return useQuery({
    queryKey: ["realtime-business-data", restaurantId],
    queryFn: async () => {
      ...
      const peakHoursData: PeakHourData[] = Object.entries(hourCounts).map(
        ([hour, data]) => ({
          hour,
      ...
      const avgCustomersPerHour =
        peakHoursData.reduce((sum, h) => sum + h.customers, 0) /
      ...
      const weekdayComparison = Object.entries(dayOfWeekData).map(
        ([day, data]) => ({
          day,
          orders: data.orders,
          revenue: data.revenue,
